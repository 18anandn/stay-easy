...
BEGIN
	...
	IF date_trunc('day', CURRENT_TIMESTAMP AT TIME ZONE home_row.time_zone) >= book_from THEN
		RAISE EXCEPTION 'The location time is past the check-in date';
	END IF;
	
	IF (date_trunc('month', (CURRENT_TIMESTAMP AT TIME ZONE home_row.time_zone) + INTERVAL '1 year' + INTERVAL '2 month') - INTERVAL '1 day')::date < book_from THEN
		RAISE EXCEPTION 'Check-in date is too far in the future';
	END IF;
