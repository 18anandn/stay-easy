...
export const getInvalidCheckiInDates = (
  bookings_by_cabins: [Date, Date][][],
  number_of_cabins: number,
): Date[] => {
  const date_counter = new Map<number, number>();
  for (const cabin of bookings_by_cabins) {
    for (const booking of cabin) {
      let start_day = booking[0];
      while (compareAsc(start_day, booking[1]) < 0) {
        date_counter.set(
          start_day.getTime(),
          (date_counter.get(start_day.getTime()) ?? 0) + 1,
        );
